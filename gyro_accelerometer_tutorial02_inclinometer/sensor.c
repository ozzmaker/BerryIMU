...
void readMAG(int  *m)
{
	...
	readBlock(0x80 | LSM303D_OUT_X_H_M, sizeof(block), block);
...
void enableIMU()
{
	...
	// Enable magnetometer
//        writeMagReg(LSM303D_MR_REG_M, 0x00);  // enable magnometer
	...
	//Enable Magnetometer
	writeMagReg( LSM303D_CTRL0, 0x00);   // Default
	writeMagReg( LSM303D_CTRL1, 0x5F);   // Acc data rate = 50Hz, BDU enable
	writeMagReg( LSM303D_CTRL2, 0x18);   // Acc full-scale = 8g
	writeMagReg( LSM303D_CTRL3, 0x00);   // INT1 disable
	writeMagReg( LSM303D_CTRL4, 0x00);   // INT2 disable
